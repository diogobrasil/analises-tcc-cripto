...
def create_window_data(
    ...
    Aplica filtro para remover colunas de preço absoluto (Open, High, etc) e manter apenas
    features estacionárias (RSI, Returns, Ranges) e Lags do Target.
    """
    ...
    # 3. Criação dos Lags do TARGET (Memória do passado)
    lags = {}
    ...
    # 4. Seleção de Features Técnicas (Tempo T)
    # AQUI ESTÁ A MUDANÇA CRÍTICA: LISTA NEGRA DE COLUNAS
    # Removemos tudo que é preço absoluto ou volume bruto para evitar vício (overfitting)
    cols_to_ban = [
        'open', 'high', 'low', 'close', 'volume', 'tick_volume', 'tickvol', 
        'spread', 'real_volume', 'time', 'date'
    ]
    
    # Pega todas as numéricas
    all_numeric = [c for c in df_proc.columns if np.issubdtype(df_proc[c].dtype, np.number)]
    
    # Filtra: Mantém se NÃO for target E NÃO estiver na lista negra (case insensitive)
    extra_features = [
        c for c in all_numeric 
        if c != target and c.lower() not in cols_to_ban
    ]
    
    # Log de segurança para você conferir no terminal o que sobrou
    logging.info(f"Features selecionadas para o modelo: {extra_features}")

    # Juntamos Lags + Features Técnicas Filtradas
    ...
    # 5. Lógica de Data para o Filtro (Cross-Day)
    ...
    # 6. Filtro Cross-Day (Overnight Gap)
    if filter_cross_day:
...
def run_training_pipeline(config: dict):
    ...
    # 5. Train (Pipeline: Scaler -> XGBoost)
    logging.info("Training XGBoost Regression Pipeline...")
    
    # XGBoost Params
    xgb_params = train_cfg.get('xgboost_params', {})
    
    # Default params if not provided, but XGBoost has good defaults.
    # We can set n_jobs=-1 to use all cores.
    ...
    # caminhos finais
    model_path = save_path / f"{target}_xgboost_model_{ver}.pkl"
    scalerY_path = save_path / f"{target}_xgboost_scalerY_{ver}.pkl"
    metadata_path = save_path / f"{target}_xgboost_metadata_{ver}.json"
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Treinamento intraday - pipeline XGBoost")
